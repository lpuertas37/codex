...
PAPERS_PROPS=['Título','Autores','Año','Revista / Fuente','DOI','URL','Abstract','Keywords','Tema principal','Marca destino','Producto relacionado','ID de tema de malla','Artículo sugerido','Nivel de ajuste','Justificación del ajuste','Relevancia 1-5','Aplicabilidad comercial 1-5','Potencial SEO 1-5','Potencial LinkedIn 1-5','Resumen ejecutivo','Implicación práctica','Estado','Fecha de búsqueda','Notas','Título normalizado']
...
def main():
    ...
    for c in raw:
        ...
        props=sanitize_props({'Título':{'title':[{'text':{'content':c.get('paper','Sin título')}}]},'Título normalizado':{'rich_text':[{'text':{'content':norm}}]},'DOI':{'rich_text':[{'text':{'content':c.get('doi','N/A')}}]},'Tema principal':{'rich_text':[{'text':{'content':c.get('linea_editorial_sugerida','')}}]},'Marca destino':{'select':{'name':c.get('marca_destino','Líderes / Skillia')}},'Producto relacionado':{'rich_text':[{'text':{'content':c.get('producto_relacionado','')}}]},'ID de tema de malla':{'rich_text':[{'text':{'content':c.get('id_tema','')}}]},'Artículo sugerido':{'rich_text':[{'text':{'content':c.get('articulo_sugerido','')}}]},'Nivel de ajuste':{'number':c.get('nivel_ajuste',0)},'Justificación del ajuste':{'rich_text':[{'text':{'content':c.get('justificacion','')}}]},'Estado':{'select':{'name':'En investigación'}}},p_allowed,rep['missing'])
        if a.dry_run: rep['skipped']+=1; continue
